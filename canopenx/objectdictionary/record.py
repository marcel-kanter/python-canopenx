...
class Record(object):
	...
	def __contains__(self, key):
		""" Returns True if the Record contains an item with the specified subindex or name.
		"""
		try:
			self[key]
		except:
	...
	def __delitem__(self, key):
		""" Removes the item identified by the name or the subindex from the Record.
		"""
		item = self[key]
	...
	def __iter__(self):
		""" Returns an iterator over all items in the Record.
		"""
		return iter(self._items_subindex.values())

	def __init__(self, name, index, data_type = 0):
		"""
		:param name: A string. The name of this Record.

		:param index: An integer. Must be in range 0x0000 .. 0xFFFF

		:param data_type: An integer. Must be in range 0x0000 .. 0x1000

		:raises: ValueError
		"""
		if index < 0x0000 or index > 0xFFFF:
			raise ValueError("The specified index is not in range 0x0000 .. 0xFFFF.")
		if data_type < 0x0000 or data_type > 0x1000:
			raise ValueError("The specified data_type is not in range 0x0000 .. 0x1000.")
	...
	def __len__(self):
		""" Returns the number of objects in the Record.
		"""
		return len(self._items_subindex)

	def add(self, item):
		""" Adds an item to the Record. It may be accessed later by the name or the subindex.
		"""
		...
		if item.subindex in self._items_subindex or item.name in self._items_name:
			raise ValueError("A item with this subindex or name is already in the Record.")
		if item.index != self.index:
			raise ValueError("The index of the item must match the index of the Record.")
